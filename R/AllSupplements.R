...
#' @export
#' 
tnsPlotGSEA2 <- function (object, aSample, regs = NULL, refsamp = NULL,
                          log = FALSE, ntop = NULL, pValueCutoff = 0.05,
                          pAdjustMethod = "BH", verbose = TRUE, ...)
{
    #-- checks
    if(class(object)!="TNS")stop("NOTE: 'tnsPlotGSEA2' requires a 'TNS' class object!")
    if(object@status["Preprocess"]!="[x]")stop("NOTE: TNS object requires preprocessing!")
    if(object@status["GSEA2"]!="[x]")stop("NOTE: TNS object needs to be evaluated by 'tnsGSEA2'!")
    ...
    if (verbose)
        cat("Transforming into TNA object\n")
    ...
    if (!is.null(regs))
    {
        ...
        if(is.null(ntop)){
            ...
            if(ntop>length(all.regs))ntop=length(all.regs)
            regs <- names(all.regs[1:ntop])
        }
    }
    ...
    if(log || LogC){
        ...
        if(LogC)warning("NOTE:'gexp' values seem not to be in log space! ..log2 transformation has been applied!")
    } else {
        dt <- gexp-gxref
    }
    ...
    if (verbose)
        cat("- Recomputing two-tailed GSEA analysis\n")
    
    rtna <- tna.gsea2(rtna, stepFilter = FALSE, pValueCutoff,
                      pAdjustMethod, tfs = regs, verbose = verbose)
    
    #-- plot
    tna.plot.gsea2(rtna, labPheno = aSample,  tfs = regs, ...)
    if (verbose)
        cat("- GSEA plot(s) for the selected sample and regulons should be 
...
.tns.stratification <- function (object,  nSections = 2)
{
    ...
    for(reg in colnames(regstatus))
    {
        ...
        tp2[]<-rep(sq+nSections+1,each=ceiling(length(tp2)/nSections),
                    ...
                    plotpdf,excludeMid,flipcols,attribs,groups,endpoint)
{
  ...
  #-- organizing plot colors
    ...
    if(nclass>1){
        ...
        #---legends
        if(nclass==2){
            legs<-paste(c("Positive dES","Negative dES")
                        [1:length(sections)],": ",res2$n," (",res2$obs,")",
                        sep="")
        } else if(nclass==3){
            legs<-paste(c("Positive dES","undetermined","Negative dES")
                        [1:length(sections)],": ",res2$n," (",res2$obs,")",
                        sep="")
        } else {
            legs<-paste("Section ",1:length(sections),": ",res2$n,"(",
                        res2$obs,")",sep="")
        }
        pval<-paste("Logrank P:",format(pval, digits=3,scientific=TRUE))
        legend("bottomleft", legend = legs, col=cols, bty="n", pch=15,cex=1,
               pt.cex=1.5)
        legend("topright", legend = pval, bty="n", adj = c(0,-0.5) )
    }
    par(op)
}
...
.plotCox = function (resall, regs, keycovar, filen, width, 
                     height, xlim, xlab, ylab, plotpdf)
{
    #--- get colors
    #ptreds<-rev(colorRampPalette(brewer.pal(9,"Reds"))(11))
    #ptblues<-rev(colorRampPalette(brewer.pal(9,"Blues"))(11))
    ...
    labs<-rownames(resall);labs[labs=="LN"]<-"LN+"
    mtext(text=labs,side=2,at=nIN:nOUT,las=2,cex=0.8)
    par(xpd=TRUE)
    legend(y=0, x=1, ncol=2,
           legend=c("covariates","associated, HR<1","not associated",
                    "associated, HR>1"),
...
.prettylog<-function(xlim){
    ...
    if(length(xlim)==2){
        ...
        tp<-range(xat)
        if(tp[1]>min(xlabs))xlabs<-c(xlabs,tp[1])
        if(tp[2]<max(xlabs))xlabs<-c(xlabs,tp[2])
        xlabs <- sort(xlabs)
    } else {
        xat<-xlabs<-xlim
    }
